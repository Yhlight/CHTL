...
public:
    ...
    bool isPropertyRequired(const std::string& property) const;
...
public:
    ...
    // 操作（add, delete）
    void addOperation(const std::string& operation, const std::string& property, const std::string& value = "");
    std::vector<std::tuple<std::string, std::string, std::string>> getOperations() const;
...
private:
    ...
    std::vector<std::tuple<std::string, std::string, std::string>> operations_; // operation, property, value
};
